...
public class MechAnimationSDX : MonoBehaviour, IAvatarController
{
    ...
    public bool isValidAndPlaying(string strAnimation)
    {
        ...
        return (IsValidAnimation(strAnimation) && this.anim.GetCurrentAnimatorStateInfo(0).IsName(strAnimation));
    ...
    protected void Update()
    {
        ...
        if (HasValidAnimationAndEnabled(this.AnimationPain))
            return;
        if (HasValidAnimationAndEnabled(this.AnimationIdle))
            return;
        if (HasValidAnimationAndEnabled(this.AnimationSecondIdle))
            return;
